...
//ADCS calculation to always have the same time for an AD conversion scan  
...
/* [JRL] Basis program test: "audio-recording" 
 * Test: the MIC_SAMP_NB value
 * 
 * The program records 2 seconds and then plays it
 * But it can also play directly a sin wave
 * attempts: to see what happens if I increase this value
 * as the recording quality is not high
 * and the sine wave sound "square"
 * -> 100 samples for 2sec... 50hz... very low... 
 * the main goal : could this be more interesting at data memory 
 * level than with the big "e_const_sound.s" and code */
...
// JRL : memory: data (ram) : 17% | Program words : 50%)
//#define MIC_SAMP_NB 256 // JRL
// JRL : memory: data (ram) : 28% | Program words : 50%)
// the sound is already better! definitely!
//#define MIC_SAMP_NB 512 // JRL
// JRL : memory: data (ram) : 47% | Program words : 50%)
// the sound is lower in frequency / 
// a latency of 1-2 seconds before emitting sound
//#define MIC_SAMP_NB 1024 // JRL
// JRL : memory: data (ram) : 85% | Program words : 50%)
// the sound is again ok (not lower), but very long latency
// before emitting sound
//
// Conclusion => definitely not a good idea to use the
// e-mic_scan array with a big MIC_SAMP_NB to just generate sound through sin wave!
